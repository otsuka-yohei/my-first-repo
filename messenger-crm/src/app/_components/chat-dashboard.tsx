...
function WorkerChatDashboard({
  ...
  const settingsButton = (
    <button
      ...
      className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-lg lg:hidden"
      aria-label="設定"
    >
      <Settings className="h-5 w-5 text-[#0F2C82]" />
    </button>
  )
      ...
      <div className="flex w-full lg:max-w-[320px] lg:flex-col lg:border-r lg:bg-white">
        <div className="flex w-full items-center justify-between bg-white px-4 py-4 lg:hidden">
          <div className="flex gap-3">
            <Button variant={mobileView === "list" ? "default" : "outline"} size="sm" onClick={() => setMobileView("list")}>
              担当者
            </Button>
            <Button variant={mobileView === "chat" ? "default" : "outline"} size="sm" onClick={() => setMobileView("chat")}>
              チャット
            </Button>
          </div>
          {settingsButton}
